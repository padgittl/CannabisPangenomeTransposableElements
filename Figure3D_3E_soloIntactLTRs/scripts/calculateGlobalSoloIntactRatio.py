    ...
    with open(unfilteredSoloLTRFile,'r') as F:
        for line in F:
            if not line.startswith('assembly'):
                ...
                soloLTRDict[ltrType][assemblyID].append((soloTEID,uniqueSoloTEID,int(soloLTRStart),int(soloLTRStop),int(alignLen)))
    return(soloLTRDict)
                                    ...
                                    if seqLen >= minAlignmentLength:
                                        ...
                                        if 'Gypsy' in feature:
                                            if 'Ty3' not in gffData:
                                                gffData['Ty3'] = {}
                                            if assemblyID not in gffData['Ty3']:
                                                gffData['Ty3'][assemblyID] = []
                                            gffData['Ty3'][assemblyID].append(ltrDist)
                                            if 'Ty3' not in gffCoordDict:
                                                gffCoordDict['Ty3'] = {}
                                            if assemblyID not in gffCoordDict['Ty3']:
                                                gffCoordDict['Ty3'][assemblyID] = []
                                            gffCoordDict['Ty3'][assemblyID].append((int(start),int(end),seqLen))
                                        elif 'Copia' in feature:
                                            if 'Ty1' not in gffData:
                                                gffData['Ty1'] = {}
                                            if assemblyID not in gffData['Ty1']:
                                                gffData['Ty1'][assemblyID] = []
                                            gffData['Ty1'][assemblyID].append(ltrDist)
                                
                                            if 'Ty1' not in gffCoordDict:
                                                gffCoordDict['Ty1'] = {}
                                            if assemblyID not in gffCoordDict['Ty1']:
                                                gffCoordDict['Ty1'][assemblyID] = []
                                            gffCoordDict['Ty1'][assemblyID].append((int(start),int(end),seqLen))
...
def calculateSoloIntactRatio(gffData,soloLTRDict,specificLTRType):
    ...
    for ltrType in gffData:
        if ltrType == specificLTRType:
            ...
            OUT.write("ltrType\tassemblyID\tsoloCount\tintactCount\tSIRatio\n")
            for assemblyID in gffData[ltrType]:
                intactCount = len(gffData[ltrType][assemblyID])
                if ltrType in soloLTRDict:
                    soloCount = len(soloLTRDict[ltrType][assemblyID])
                    SIRatio = float(soloCount) / intactCount
                    # print(ltrType,assemblyID,soloCount,intactCount,SIRatio)
                    OUT.write("%s\t%s\t%s\t%s\t%s\n" % (ltrType,assemblyID,soloCount,intactCount,SIRatio))
                    if assemblyID not in dataDict:
                        dataDict[assemblyID] = []
                    dataDict[assemblyID].append(SIRatio)
    return(dataDict)
