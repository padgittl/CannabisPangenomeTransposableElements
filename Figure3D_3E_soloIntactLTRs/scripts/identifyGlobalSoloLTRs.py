        ...
        for line in F:
            if not line.startswith('#'):
                ...
                if 'chr' in scaffoldID:
                    ...
                    if 'INT' not in countDict['Gypsy'][assemblyID]:
                        countDict['Gypsy'][assemblyID]['INT'] = 0
                    if 'LTR' not in countDict['Gypsy'][assemblyID]:
                        countDict['Gypsy'][assemblyID]['LTR'] = 0
                    ### 
                    ### 
                    ...
                    if 'INT' not in countDict['Copia'][assemblyID]:
                        countDict['Copia'][assemblyID]['INT'] = 0
                    if 'LTR' not in countDict['Copia'][assemblyID]:
                        countDict['Copia'][assemblyID]['LTR'] = 0
                                        ...
                                        if percIden >= minPercentIdentity:
                                            ...
                                            if '_INT' not in teID:
                                                ...
                                                if 'Gypsy' in feature:
                                                    ...
                                                    countDict['Gypsy'][assemblyID]['LTR'] += 1

                                                    if 'Gypsy' not in candidateSoloLTRCoordDict:
                                                        candidateSoloLTRCoordDict['Gypsy'] = {}
                                                    if assemblyID not in candidateSoloLTRCoordDict['Gypsy']:
                                                        candidateSoloLTRCoordDict['Gypsy'][assemblyID] = []
                                                    candidateSoloLTRCoordDict['Gypsy'][assemblyID].append((int(start),int(end),alignLen,teID,uniqueTEID,strand))
                                                # print(start,end,alignLen,teID,uniqueTEID)
                                                    ...
                                                    countDict['Copia'][assemblyID]['LTR'] += 1
                                                    ...
                                                    if assemblyID not in candidateSoloLTRCoordDict['Copia']:
                                                        candidateSoloLTRCoordDict['Copia'][assemblyID] = []
                                                    candidateSoloLTRCoordDict['Copia'][assemblyID].append((int(start),int(end),alignLen,teID,uniqueTEID,strand))
                                                    '''
                                            ...
                                            if '_INT' in teID:
                                                # print(teID)
                                                if 'Gypsy' in feature:
                                                    countDict['Gypsy'][assemblyID]['INT'] += 1
                                                    if 'Gypsy' not in internalFeatureCoordDict:
                                                        internalFeatureCoordDict['Gypsy'] = {}
                                                    if assemblyID not in internalFeatureCoordDict['Gypsy']:
                                                        internalFeatureCoordDict['Gypsy'][assemblyID] = []
                                                    internalFeatureCoordDict['Gypsy'][assemblyID].append((int(start),int(end),alignLen,teID,uniqueTEID,strand))
                                                    ...
                                                    countDict['Copia'][assemblyID]['INT'] += 1
                                                    ...
                                                    if assemblyID not in internalFeatureCoordDict['Copia']:
                                                        internalFeatureCoordDict['Copia'][assemblyID] = []
                                                    internalFeatureCoordDict['Copia'][assemblyID].append((int(start),int(end),alignLen,teID,uniqueTEID,strand))
                                    ...
                                    if alignLen >= minAlignmentLength:
                                        if percIden >= minPercentIdentity:
                                            if 'Gypsy' in feature:
                                                ...
                                                if assemblyID not in intactLTRCoordDict['Gypsy']:
                                                    intactLTRCoordDict['Gypsy'][assemblyID] = []
                                                intactLTRCoordDict['Gypsy'][assemblyID].append((int(start),int(end),alignLen,teID,uniqueTEID,strand))
                                            elif 'Copia' in feature:
                                                if 'Copia' not in intactLTRCoordDict:
                                                    intactLTRCoordDict['Copia'] = {}
                                                if assemblyID not in intactLTRCoordDict['Copia']:
                                                    intactLTRCoordDict['Copia'][assemblyID] = []
                                                intactLTRCoordDict['Copia'][assemblyID].append((int(start),int(end),alignLen,teID,uniqueTEID,strand))
                                                
    for ltrType in countDict:
        for assemblyID in countDict[ltrType]:
            intCount = countDict[ltrType][assemblyID]['INT']
            ltrCount = countDict[ltrType][assemblyID]['LTR']
...
def removeMatchingAdjacentEntries(candidateSoloLTRCoordDict):
    ...
    for ltrType in candidateSoloLTRCoordDict:
        ...
        if ltrType != 'Other':
            for assemblyID in candidateSoloLTRCoordDict[ltrType]:
                # sort by start position, ascending
                candidateSoloLTRCoordDict[ltrType][assemblyID].sort(key = lambda x:x[0], reverse=False)
                # the "-2" is to allow i+1 and i+2 searches
                for i in range(len(candidateSoloLTRCoordDict[ltrType][assemblyID])-4):
                    # 4 upstream indexes
                    start_n1,end_n1,alignLen_n1,teID_n1,uniqueTEID_n1,strand_n1 = candidateSoloLTRCoordDict[ltrType][assemblyID][i-1]
                    start_n2,end_n2,alignLen_n2,teID_n2,uniqueTEID_n2,strand_n2 = candidateSoloLTRCoordDict[ltrType][assemblyID][i-2]
                    start_n3,end_n3,alignLen_n3,teID_n3,uniqueTEID_n3,strand_n3 = candidateSoloLTRCoordDict[ltrType][assemblyID][i-3]
                    start_n4,end_n4,alignLen_n4,teID_n4,uniqueTEID_n4,strand_n4 = candidateSoloLTRCoordDict[ltrType][assemblyID][i-4]
                        
                    # i -- current pos
                    start0,end0,alignLen0,teID0,uniqueTEID0,strand0 = candidateSoloLTRCoordDict[ltrType][assemblyID][i]
                        
                    # two downstream indexes
                    start1,end1,alignLen1,teID1,uniqueTEID1,strand1 = candidateSoloLTRCoordDict[ltrType][assemblyID][i+1]
                    start2,end2,alignLen2,teID2,uniqueTEID2,strand2 = candidateSoloLTRCoordDict[ltrType][assemblyID][i+2]
                    # comment out these two except for testing
                    start3,end3,alignLen3,teID3,uniqueTEID3,strand3 = candidateSoloLTRCoordDict[ltrType][assemblyID][i+3]
                    start4,end4,alignLen4,teID4,uniqueTEID4,strand4 = candidateSoloLTRCoordDict[ltrType][assemblyID][i+4]
                    ...
                    if teID0 != teID1 and teID0 != teID2 and teID0 != teID3 and teID0 != teID4:
                        ...
                        if assemblyID not in downstreamMatchDict[ltrType]:
                            downstreamMatchDict[ltrType][assemblyID] = {}
                        if uniqueTEID0 not in downstreamMatchDict[ltrType][assemblyID]:
                            downstreamMatchDict[ltrType][assemblyID][uniqueTEID0] = (start0,end0,alignLen0,teID0,strand0)
                        ...
                        if start_n1 < start0 and end_n1 < start0 and start_n2 < start0 and end_n2 < start0 and start_n3 < start0 and end_n3 < start0 and start_n4 < start0 and end_n4 < start0:
                            ...
                            if teID0 != teID_n1 and teID0 != teID_n2 and teID0 != teID_n3 and teID0 != teID_n4:
                                ...
                                if assemblyID not in upstreamMatchDict[ltrType]:
                                    upstreamMatchDict[ltrType][assemblyID] = {}
                                if uniqueTEID0 not in upstreamMatchDict[ltrType][assemblyID]:
                                    upstreamMatchDict[ltrType][assemblyID][uniqueTEID0] = (start0,end0,alignLen0,teID0,strand0)
