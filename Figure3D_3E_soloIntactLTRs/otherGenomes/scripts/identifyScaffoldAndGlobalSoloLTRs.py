...
def readScaffoldLengthsFile(scaffoldLengthsFile,specificAssemblyID,organismID):
    chrLengthsDict = {}
    with open(scaffoldLengthsFile,'r') as F:
        for line in F:
            ...
            if chrID not in chrLengthsDict[assemblyID]:
                chrLengthsDict[assemblyID][chrID] = scaffoldLength
    return(chrLengthsDict)


def readGeneData(gffFile,specificAssemblyID,organismID):
    chrLengthsDict = {}
...
def assessCandidateSoloLTRBoundariesWithIntactLTRs(chrLengthsDict,intactLTRCoordDict,candidateSoloLTRCoordDict,downstreamMatchDict,upstreamMatchDict,ltrType,soloWindowBoundary):
    ...
    for assemblyID in chrLengthsDict:
        for chrID in chrLengthsDict[assemblyID]:
            ...
            if ltrType in intactLTRCoordDict:
                for chromID in intactLTRCoordDict[ltrType]:
                    if chrID == chromID:
                        # print(chrID,chromID)
                        if assemblyID in intactLTRCoordDict[ltrType][chrID]:
                            ...
                            for intactStart,intactStop,intactAlignLen,intactTEID,uniqueIntactTEID,intactStrand in intactLTRCoordDict[ltrType][chrID][assemblyID]:
                                #print(assemblyID,chrID,chrLength,intactStart,intactStop,intactAlignLen,intactTEID,uniqueIntactTEID,intactStrand)
                                if intactStart < intactStop:
                                    if intactStop > chrLength:
                                        print("check coordinates",assemblyID,chrID,ltrType,chrLength,uniqueIntactTEID,intactStart,intactStop)
                                        sys.exit()
                                    else:
                                        #print("assemblyID,chrID,chrLength,ltrType,intactStart,intactStop,intactAlignLen,intactTEID,uniqueIntactTEID,intactStrand")
                                        #print(assemblyID,chrID,chrLength,ltrType,intactStart,intactStop,intactAlignLen,intactTEID,uniqueIntactTEID,intactStrand)
                                        #continue
            ...
            if ltrType in candidateSoloLTRCoordDict:
                for chromID in candidateSoloLTRCoordDict[ltrType]:
                    if chrID == chromID:
                        if assemblyID in candidateSoloLTRCoordDict[ltrType][chrID]:
                            ...
                            for soloLTRStart,soloLTRStop,soloAlignLen,soloTEID,uniqueSoloTEID,soloStrand in candidateSoloLTRCoordDict[ltrType][chrID][assemblyID]:
                                if soloLTRStart < soloLTRStop:
                                    if soloLTRStop > chrLength:
                                        print("assemblyID,chrID,ltrType,chrLength,uniqueIntactTEID,intactStart,intactStop")
                                        print("check coordinates",assemblyID,chrID,ltrType,chrLength,uniqueIntactTEID,intactStart,intactStop)
                                        sys.exit()
                                    else:
                                        #print("assemblyID,chrID,chrLength,ltrType,soloLTRStart,soloLTRStop,soloAlignLen,soloTEID,uniqueSoloTEID,soloStrand")
                                        #print(assemblyID,chrID,chrLength,ltrType,soloLTRStart,soloLTRStop,soloAlignLen,soloTEID,uniqueSoloTEID,soloStrand)
                                ...
                                if len(soloWindow5PrimeSet) == 1:
                                    #print(assemblyID,chrID,soloWindow5PrimeSet)
                                    if list(soloWindow5PrimeSet)[0] == 0:
                                        # print(assemblyID,chrID,soloWindow5PrimeSet)
                                        if len(soloWindow3PrimeSet) == 1:
                                            #print(assemblyID,chrID,soloWindow3PrimeSet)
                                            if list(soloWindow3PrimeSet)[0] == 0:
                                                if ltrType in downstreamMatchDict and ltrType in upstreamMatchDict:
                                                    for down_chromID in downstreamMatchDict[ltrType]:
                                                        if chrID == down_chromID:
                                                            for up_chromID in upstreamMatchDict[ltrType]:
                                                                if chrID == up_chromID:
                                                                    #if chrID == downstreamMatchDict[ltrType] and chrID == upstreamMatchDict[ltrType]:
...
def assessCandidateSoloLTRBoundariesWithInternalSequences(chrLengthsDict,candidateSoloLTRCoordDict,internalFeatureCoordDict,downstreamMatchDict,upstreamMatchDict,ltrType,soloWindowBoundary):
    ...
    for assemblyID in chrLengthsDict:
        for chrID in chrLengthsDict[assemblyID]:
            ...
            if ltrType in internalFeatureCoordDict:
                for chromID in internalFeatureCoordDict[ltrType]:
                    if chrID == chromID:
                        if assemblyID in internalFeatureCoordDict[ltrType][chrID]:
...
def writeCandidateSoloLTRs(candidateSoloLTRDataList,p,ltrType,strand,histBinWidth,specificAssemblyID,soloWindowBoundary):
    ...
    for assemblyID,chrID,soloTEID,uniqueSoloTEID,soloLTRStart,soloLTRStop,alignLen,soloStrand in candidateSoloLTRDataList:
        if alignLen <= p:
            if assemblyID not in globalData:
                globalData[assemblyID] = {}
            if uniqueSoloTEID not in globalData[assemblyID]:
                globalData[assemblyID][uniqueSoloTEID] = 1
            if assemblyID not in globalLength:
                globalLength[assemblyID] = 0
            globalLength[assemblyID] += alignLen
            OUT.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (assemblyID,chrID,newLTRType,soloTEID,uniqueSoloTEID,soloLTRStart,soloLTRStop,alignLen,soloStrand))
    OUT_TOTAL = open(specificAssemblyID + '_' + newLTRType + '_' + strand + '_binWidth' + str(histBinWidth) + '_soloLTRFlankingWindow' + str(soloWindowBoundary) + '_globalCandidateSoloLTRData.tsv','w')
    OUT_TOTAL.write("# 95th percentile: %s\n" % (p))
    OUT_TOTAL.write("# Bin width: %s\n" % (histBinWidth))
    OUT_TOTAL.write("# Solo-LTR flanking window length: %s\n" % (soloWindowBoundary))
    OUT_TOTAL.write("# assemblyID\tsoloLTRCount\tsoloLTRLength\n")
    for assemblyID in globalData:
        numberSoloLTRs = len(globalData[assemblyID])
        soloLTRLength = globalLength[assemblyID]
        OUT_TOTAL.write("%s\t%s\t%s\n" % (assemblyID,numberSoloLTRs,soloLTRLength))
...
usage = "Usage: " + sys.argv[0] + " <full TE anno gff file> <gene gff file> <scaffold lengths file> <distance histogram bin width> <assemblyID> <species or organism ID, i.e. cannabis or maize> <solo-LTR flanking window value, i.e. either 300 bp (LTR_retriever) or 5000 bp (LTR_MINER)>\n"
if len(sys.argv) != 8:
    print(usage)
    sys.exit()
...
geneGFF = sys.argv[2]
scaffoldLengthsFile = sys.argv[3]
histBinWidth = sys.argv[4]
assemblyID = sys.argv[5]
organismID = sys.argv[6]
soloLTRFlankingWindow = sys.argv[7]
...
if organismID == 'cannabis':
    chrLengthsDict = readGeneData(geneGFF,assemblyID,organismID)
if organismID == 'other':
    chrLengthsDict = readScaffoldLengthsFile(scaffoldLengthsFile,assemblyID,organismID)
elif organismID == 'maize':
...
#print(plus_gypsyFilteredCandidateSoloLTRCoordDict1)
...
#print(plus_gypsyFilteredCandidateSoloLTRCoordDict2)
...
#print(plus_gypsyCandidateSoloLTRLengthList,plus_gypsyCandidateSoloLTRDataList)
...
#print(combinedGypsyLengthList)
#print(combinedGypsyDataList)
